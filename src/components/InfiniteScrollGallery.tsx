  ...
  return (
    <section ref={containerRef} className="py-20 overflow-hidden">
      ...
      <div className="relative">
        ...
        <div 
          className={`flex gap-8 animate-${animationDirection} cinematic-scroll infinite-scroll-optimized group/scroll`}
          style={{
        ...
        >
          {duplicatedItems.map((item, index) => (
            <motion.div
              ...
              transition={{ 
                duration: 0.6, 
                delay: (index % items.length) * 0.1,
                ease: "easeOut"
              }}
              className="flex-shrink-0 w-80 md:w-96 group cursor-pointer gallery-card-hover"
              ...
              <div className="relative overflow-hidden rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 bg-card border border-border/50 hover:border-teal-200 dark:hover:border-teal-800">
                ...
                <div className="relative h-64 md:h-80 overflow-hidden">
                  <ImageWithFallback
                    src={item.image}
                    alt={item.title}
                    className="w-full h-full object-cover gallery-image-zoom"
                  />
                  ...
                  {/* Hover Overlay */}
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 gallery-overlay">
